...
When everything is created, ensure ArgoCD recognizes the existing deployment, and doesn't try to create new resources. When Argo has taken over, you can uncomment the `syncPolicy: automated: {}` section in the `argo/zone2` resources, to automatically sync changes.
